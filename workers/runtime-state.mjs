...
function serializeState(state) {
  return {
    items: state.items,
    nextQuoteId: state.nextQuoteId,
    nextPurchaseId: state.nextPurchaseId,
    nextClaimId: state.nextClaimId,
    nextOnChainJobId: state.nextOnChainJobId,
    nextAcpJobId: state.nextAcpJobId,
    quotes: Array.from(state.quotes.entries()),
    purchases: Array.from(state.purchases.entries()),
    claims: Array.from(state.claims.entries()),
    sellerOutcomes: Array.from(state.sellerOutcomes.entries()),
  };
}
...
function deserializeState(rawState) {
  ...
  return {
    items: Array.isArray(rawState.items) ? rawState.items : clone(DEFAULT_ITEMS),
    nextQuoteId: Number(rawState.nextQuoteId ?? 1),
    nextPurchaseId: Number(rawState.nextPurchaseId ?? 1),
    nextClaimId: Number(rawState.nextClaimId ?? 1),
    nextOnChainJobId: Number(rawState.nextOnChainJobId ?? 6001),
    nextAcpJobId: Number(rawState.nextAcpJobId ?? 9001),
    quotes: new Map(rawState.quotes ?? []),
    purchases: new Map(rawState.purchases ?? []),
    claims: new Map(rawState.claims ?? []),
    sellerOutcomes: new Map(rawState.sellerOutcomes ?? []),
  };
}
...
export class RuntimeState {
  constructor(ctx) {
    this.ctx = ctx;
    this.state = null;
    this.statePromise = this.ctx.storage.get('state').then((stored) => deserializeState(stored));
  }
  ...
  async fetch(request) {
    ...
    try {
      ...
      if (request.method === 'POST' && url.pathname === '/api/risk/quote/intel') {
        ...
        const quote = createQuote(state, item, buyerAgentId);
        await this.persist();
        return json(quote);
      }
