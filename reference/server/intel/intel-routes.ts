...
import {
  createIntelPurchaseJob,
  completeIntelPurchase,
  submitIntelPurchaseDelivery,
} from '../erc8183/hooks/intel-hook.js';
...
import {
  createProtectedIntelPurchaseRecord,
  updateProtectedIntelPurchaseRecord,
} from '../risk/protected-purchase.js';
import { getRiskQuoteById } from '../risk/quote-engine.js';
...
router.post('/items/:id/buy', async (req, res) => {
  try {
    const pool = getPool();
    const { buyerAgentId, settlementMode, purchaseMode, quoteId } = req.body as {
      buyerAgentId?: string;
      settlementMode?: 'record_only' | 'acp_funded';
      purchaseMode?: 'instant' | 'challengeable';
      quoteId?: number;
    };
    const itemId = parseInt(req.params.id);
    const useChallengeable = purchaseMode === 'challengeable';
    const useFundedEscrow = useChallengeable || settlementMode === 'acp_funded';
    ...
    if (useChallengeable) {
      if (!quoteId) {
        return res.status(400).json({ error: 'quoteId required for challengeable purchase' });
      }
      riskQuote = await getRiskQuoteById(quoteId);
      if (!riskQuote) {
        return res.status(404).json({ error: 'Risk quote not found' });
      }
      if (riskQuote.intelItemId !== itemId || riskQuote.buyerAgentId !== buyerAgentId) {
        return res.status(400).json({ error: 'Risk quote does not match this buyer or intel item' });
      }
      if (new Date(riskQuote.expiresAt).getTime() < Date.now()) {
        return res.status(400).json({ error: 'Risk quote expired' });
      }
    }
    ...
    let protectedPurchaseId: number | null = null;
    let challengeDeadline: string | null = null;
    let evaluatorAddress: string | null = null;
    const challengeDeadlineDate = new Date(Date.now() + (riskQuote?.claimWindowSeconds ?? 1800) * 1000);
    if (useChallengeable) {
      const protectedPurchase = await withTransaction(async (client) => createProtectedIntelPurchaseRecord({
        quoteId: riskQuote?.id ?? null,
        intelItemId: itemId,
        intelPurchaseId: null,
        buyerAgentId,
        sellerAgentId: item.producer_agent_id,
        mode: 'challengeable',
        principalAmount: price,
        premiumAmount: riskQuote?.premiumAmount ?? 0,
        principalAcpJobId: acpJobId,
        deliverableHash: contentHash,
        challengeDeadline: challengeDeadlineDate,
        status: 'pending_purchase_record',
        metadata: {
          quotedRiskScore: riskQuote?.riskScore ?? null,
          riskReasons: riskQuote?.reasons ?? [],
          premiumCollection: 'quoted_only',
          purchaseRecordState: 'pending',
          deliverySubmissionState: 'pending',
        },
      }, client));

      protectedPurchaseId = protectedPurchase.id;
      challengeDeadline = challengeDeadlineDate.toISOString();
      const acpJobRow = await pool.query<{ evaluator_address: string | null }>(
        `SELECT evaluator_address
         FROM acp_jobs
         WHERE id = $1`,
        [acpJobId],
      );
      evaluatorAddress = acpJobRow.rows[0]?.evaluator_address ?? null;
    }

    let currentTick = 0;
    let newBuyerCount = 0;
    let isConsensusSealed = false;
    let publicAfterTick: number | null = null;
    let intelPurchaseId = 0;
    ...
    try {
      ({
        ...
        const purchaseResult = await client.query<{ id: number }>(
        ...
        return {
          ...
          intelPurchaseId: purchaseResult.rows[0].id,
        };
      }));
    } catch (error) {
      if (protectedPurchaseId != null) {
        await updateProtectedIntelPurchaseRecord({
          protectedPurchaseId,
          status: 'purchase_failed',
          metadata: {
            purchaseRecordState: 'failed',
            purchaseRecordError: error instanceof Error ? error.message : String(error),
          },
        }).catch(() => undefined);
      }
      throw error;
    ...
    if (useChallengeable) {
      await updateProtectedIntelPurchaseRecord({
        protectedPurchaseId: protectedPurchaseId!,
        intelPurchaseId,
        status: 'pending_delivery',
        metadata: {
          purchaseRecordState: 'recorded',
          purchaseRecordedAt: new Date().toISOString(),
        },
      });

      try {
        await submitIntelPurchaseDelivery({
          acpJobId,
          itemId,
        ...
        await updateProtectedIntelPurchaseRecord({
          ...
          metadata: {
            deliverySubmissionState: 'failed',
    ...
    eventBus.emit('intel_v2_purchased', {
      ...
      onChainJobId,
      settlementMode: useFundedEscrow ? 'acp_funded' : 'record_only',
      purchaseMode: useChallengeable ? 'challengeable' : 'instant',
    ...
    res.json({
      ...
      quote_id: riskQuote?.id ?? null,
      intel_item_id: itemId,
      buyer_agent_id: buyerAgentId,
      seller_agent_id: item.producer_agent_id,
      status: useChallengeable ? 'challenge_window' : 'released',
      principal_amount: price.toFixed(6),
      premium_amount: (riskQuote?.premiumAmount ?? 0).toFixed(6),
