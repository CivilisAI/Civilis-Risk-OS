...
export async function completeIntelPurchase(params: {
  ...
  const { acpJobId, itemId, producerAgentId, category, contentHash } = params;
  ...
  await finalizeIntelPurchaseSettlement({
    acpJobId,
    outcome: 'release',
    reason: `purchased_${itemId}`,
  });
}

async function requestIntelValidationIfMissing(params: {
  itemId: number;
  producerAgentId: string;
  category: string;
  contentHash: string;
}): Promise<void> {
  const { itemId, producerAgentId, category, contentHash } = params;
  const pool = getPool();
...
export async function submitIntelPurchaseDelivery(params: {
  acpJobId: number;
  itemId: number;
  producerAgentId: string;
  category: string;
  ...
  const jobRow = await pool.query<{ metadata: Record<string, unknown> | null; status: string }>(
...
export async function finalizeIntelPurchaseSettlement(params: {
  acpJobId: number;
  outcome: 'release' | 'refund';
  reason: string;
}): Promise<void> {
  const { acpJobId, outcome, reason } = params;
  const acp = getACPClient();
  const pool = getPool();
  ...
  if (outcome === 'release') {
    ...
    if (status !== 'submitted') {
      throw new Error(`ACP job ${acpJobId} cannot be released from status ${status}`);
    }
  ...
  if (status !== 'submitted') {
    throw new Error(`ACP job ${acpJobId} cannot be refunded from status ${status}`);
  }
  await acp.rejectJob(acpJobId, reason);
}
