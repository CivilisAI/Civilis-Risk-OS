...
const SCHEMA_QUERIES = [
  ...
  `DO $$ BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='x402_transactions' AND column_name='onchain_status') THEN
...
const MIGRATIONS = [
  ...
  `DO $$ BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name='intel_items') THEN
      ...
      CREATE INDEX IF NOT EXISTS idx_intel_items_public_after ON intel_items(public_after_tick)
      WHERE public_after_tick IS NOT NULL AND is_public = false;
    END IF;
  END $$`,
  ...
  `CREATE TABLE IF NOT EXISTS risk_quotes (
    id SERIAL PRIMARY KEY,
    domain VARCHAR(30) NOT NULL DEFAULT 'intel_purchase',
    intel_item_id INTEGER NOT NULL REFERENCES intel_items(id),
    buyer_agent_id TEXT NOT NULL REFERENCES agents(agent_id),
    seller_agent_id TEXT NOT NULL REFERENCES agents(agent_id),
    ...
    premium_amount DECIMAL(20,6) NOT NULL,
  ...
  `CREATE INDEX IF NOT EXISTS idx_risk_quotes_item_buyer ON risk_quotes(intel_item_id, buyer_agent_id, created_at DESC)`,
  `CREATE INDEX IF NOT EXISTS idx_risk_quotes_expiry ON risk_quotes(expires_at)`,

  `CREATE TABLE IF NOT EXISTS protected_intel_purchases (
    id SERIAL PRIMARY KEY,
    quote_id INTEGER REFERENCES risk_quotes(id),
    intel_item_id INTEGER NOT NULL REFERENCES intel_items(id),
    intel_purchase_id INTEGER REFERENCES intel_purchases(id),
    buyer_agent_id TEXT NOT NULL REFERENCES agents(agent_id),
    seller_agent_id TEXT NOT NULL REFERENCES agents(agent_id),
    ...
    principal_acp_job_id INTEGER NOT NULL,
  ...
  `CREATE INDEX IF NOT EXISTS idx_protected_intel_status_deadline ON protected_intel_purchases(status, challenge_deadline)`,
  `CREATE INDEX IF NOT EXISTS idx_protected_intel_seller ON protected_intel_purchases(seller_agent_id, created_at DESC)`,
  `CREATE INDEX IF NOT EXISTS idx_protected_intel_buyer ON protected_intel_purchases(buyer_agent_id, created_at DESC)`,

  `CREATE TABLE IF NOT EXISTS purchase_claims (
    id SERIAL PRIMARY KEY,
    protected_purchase_id INTEGER NOT NULL REFERENCES protected_intel_purchases(id),
    claimant_agent_id TEXT NOT NULL REFERENCES agents(agent_id),
  ...
  `DO $$ BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='acp_jobs' AND column_name='protocol_version') THEN
