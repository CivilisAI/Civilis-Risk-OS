...
class ValidationRegistryClient {
   ...
   */
  async getProducerValidationSummary(
    tokenId: number,
    ...
    if (this.isConfigured() && options?.localOnly !== true) {
      try {
  ...
  async getProducerValidationView(
    tokenId: number,
    options?: { localOnly?: boolean },
  ): Promise<{
    ...
    const local = await this.getProducerValidationSummary(tokenId, options);
