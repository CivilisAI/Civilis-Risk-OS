...
export const FEEDBACK_TAGS = {
  ...
  INTEL_PROTECTED_RELEASE: { tag1: 'intel_protected_release', tag2: 'intel_commerce' },
  INTEL_PROTECTED_REFUND: { tag1: 'intel_protected_refund', tag2: 'intel_commerce' },
...
class ReputationRegistryClient {
  ...
  async reportProtectedIntelOutcome(params: {
    buyerAgentId: string;
    producerAgentId: string;
    producerTokenId: number;
    ...
    const { buyerAgentId, producerAgentId, producerTokenId, outcome, protectedPurchaseId, claimId } = params;
    const tags = outcome === 'refund'
      ? FEEDBACK_TAGS.INTEL_PROTECTED_REFUND
      : FEEDBACK_TAGS.INTEL_PROTECTED_RELEASE;
    const clientAddress = await getAgentWalletAddressStrict(buyerAgentId).catch(() => undefined);

    this.queueFeedback({
      agentId: producerAgentId,
      erc8004TokenId: producerTokenId,
      value: outcome === 'refund' ? -80 : 60,
      valueDecimals: 0,
      clientAddress,
      ...tags,
      metadata: {
    ...
    if (localOnly) {
      const clientCount =
        (local.onChainCount ?? 0) > 0
          ? (await this.getTrackedOnChainClients(tokenId)).length
          : 0;
      const onChainSummary =
        (local.onChainCount ?? 0) > 0
          ? {
              count: local.onChainCount ?? 0,
              averageValue: local.onChainAverageValue ?? 0,
              clientScope: 'tracked_onchain_clients' as const,
              clientCount,
            }
          : null;
      ...
      return {
        ...
        onChainSummary,
        syncState: onChainSummary ? 'mixed' : (local.count > 0 ? 'local_only' : 'empty'),
      };
    }
