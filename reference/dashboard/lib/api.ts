...
export type IntelProtectionPurchaseMode = 'auto' | 'instant' | 'challengeable'
export type IntelProtectionDecision = 'release' | 'refund'
export type IntelProtectionWorkspace = 'buyer' | 'evaluator'

const INTEL_PROTECTION_FINAL_STATUSES = new Set([
  'released',
  'settled',
  'complete',
  'completed',
...
export function isIntelProtectionFinalStatus(status?: string | null) {
  return INTEL_PROTECTION_FINAL_STATUSES.has((status ?? '').toLowerCase())
}

export function describeIntelProtectionWorkspace(workspace: IntelProtectionWorkspace, zh: boolean) {
  if (workspace === 'buyer') {
    return zh ? '买方工作台' : 'Buyer Workspace'
  }

  return zh ? '评审工作台' : 'Evaluator Workspace'
}

export interface IntelProtectionQuote {
  quote_id: number
  intel_item_id: number
  buyer_agent_id: string
  seller_agent_id: string
  risk_score: number
  recommended_mode: Exclude<IntelProtectionPurchaseMode, 'auto'>
  premium_bps: number
...
export interface IntelProtectionClaim {
  claim_id: number
  ...
  decision?: IntelProtectionDecision | null
...
export interface IntelProtectedPurchase {
  ...
  buyer_agent_id: string
  seller_agent_id: string
  purchase_mode: Exclude<IntelProtectionPurchaseMode, 'auto'>
  status: string
  principal_amount: string
  premium_amount: string
  acp_job_id?: number | null
  evaluator_address?: string | null
  challenge_deadline?: string | null
  purchase_tx_hash?: string | null
  premium_tx_hash?: string | null
  settled_at?: string | null
  created_at?: string
  claim?: IntelProtectionClaim | null
}
...
export const api = {
  ...
  postIntelProtectionQuote: (body: {
    intelItemId: number
    buyerAgentId: string
    purchaseMode?: IntelProtectionPurchaseMode
  }) =>
    postJSON<IntelProtectionQuote>('/api/risk/quote/intel', body),
  postIntelProtectedBuy: (
    itemId: number,
    body: {
      buyerAgentId: string
      purchaseMode: Exclude<IntelProtectionPurchaseMode, 'auto'>
      quoteId: number
    },
  ) =>
    postJSON<IntelProtectedPurchase>(`/api/intel/items/${itemId}/buy`, body),
  getIntelProtectedPurchase: (purchaseId: number) =>
    getJSON<IntelProtectedPurchase>(`/api/risk/purchases/${purchaseId}`),
  postIntelProtectionClaim: (body: {
